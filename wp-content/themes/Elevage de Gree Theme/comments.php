<!--Pour les commentaires d'articles -->
                ...
                <li class="<?php echo $oddcomment; ?>" id="comment-<?php comment_ID() ?>">
                    ...
                    <div class="commentmetadata">
                        <strong><?php comment_author_link() ?></strong>, <?php _e('le'); ?> <a
                            ...
                            title=""><?php comment_date('j F, Y') ?><?php _e('&agrave;'); ?><?php comment_time() ?></a> <?php _e('Vous pouvez &#58;'); ?> <?php edit_comment_link('Modifier le commentaire', '', ''); ?>
    ...
    <?php else : ?>
        ...
        <form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform">
            <?php if ($user_ID) : ?>
                ...
                <p>Vous &ecirc;tes connect&eacute; en tant que <a
            ...
            <?php else : ?>
                ...
                <p><input type="text" name="author" id="author" value="<?php echo $comment_author; ?>" size="40"
                          tabindex="1"/>
                    <label for="author">
                        <small>Nom <?php if ($req) echo "(requis)"; ?></small>
                    </label></p>

                <p><input type="text" name="email" id="email" value="<?php echo $comment_author_email; ?>" size="40"
                          tabindex="2"/>
                    <label for="email">
                        <small>email (ne sera pas publi&eacute;) <?php if ($req) echo "(requis)"; ?></small>
                    </label></p>
